...
const int n_interactions = 100; // Number of pairwise interactions between colonies per generation
...
int main(int argc, char *argv[]) {
    ...
    if (coop_enabled == true) {
        resultsFile.open("MaxMeanFit_2_1_Coop.csv");
    } else {
        resultsFile.open("2_3_4C_TBGA_short.csv");
    }
    resultsFile.close();
...
#pragma omp parallel for shared(ga1, ga2, ga3, ga4, thread_current_step) schedule(dynamic,1)
...
//            Colony * colonies[] = {col1, col2};
            ...
            float previous_seeds_collected[] = {col1->seeds_collected, col2->seeds_collected};
         ...
         */
        if (coop_enabled == true) {
            resultsFile.open("MaxMeanFit_2_1_Coop.csv", ios::out | ios::app);
        } else {
            resultsFile.open("2_3_4C_TBGA_short.csv", ios::out | ios::app);
        }
